...
**The create is conditional, too.** Once Phase 4 exposed `POST /subscribe` publicly, a bare
`PutItem` became a way to attack an existing subscriber: posting their address would overwrite
their `CONFIRMED` item with a fresh `PENDING` one and silently stop their alerts. So the write
carries `attribute_not_exists(pk) OR status IN (PENDING, UNSUBSCRIBED)` and
`create_pending_subscriber()` returns `None` when it fails. Re-posting a still-`PENDING`
address *is* allowed and re-issues the token — that's what makes "resend the confirmation
email" work without a separate endpoint.
